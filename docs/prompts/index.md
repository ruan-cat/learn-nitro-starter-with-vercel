...
1. 在 package.json 内的 homepage 字段内，增加 `https://learn-nitro-starter-with-vercel.vercel.app/` ，这是我们的生产环境可访问地址。
...
1. 开启这个功能；
2. 我要求以直接访问 `https://<baseUrl>/openapi.json` 的方式，就能在生产环境内直接访问到具体的实体 json 文件，直接就能看到公开的数据文件。
3. 开发环境要有，生产环境要能直接看。
